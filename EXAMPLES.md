...
- ✅ **SUCCESS** - No issues or only informational findings
...
Not detected:
  • (No explicit checks for absent items in this run)
...
---
ℹ️  This report explains this incident only.
...
Detected:
  • [cost] Model pricing: $30/1M input tokens, $60/1M output tokens
...
Not detected:
  • Streaming stalls (TTFB < 5s)
  • Chunk gaps (max gap < 10s)
...
Detected:
  • [streaming] TTFB: 6.7s (threshold: 5s)

Not detected:
  • Chunk gaps (max gap < 10s threshold)

Not observable from provider probe:
  • Whether client retries after partial stream
  • Stream truncation behavior
...
✓ Detected: https://api.custom-provider.com/v1
✓ Provider: openai-compatible
...
Detected:
  • [streaming] Max chunk gap: 35.2s (>30s threshold)
  • [streaming] TTFB: 7.1s (threshold: 5s)
...
Not detected:
  • Rate limiting (no 429s in 1 probe)
...
Detected:
  • [retries] Rate limiting detected (HTTP 429)
  • [retries] Retry-After header: 20s
...
Detected:
  • [retries] Rate limit remaining: 5 requests
...
🔬 Running trace checks...


✅ Status: SUCCESS

Detected:
  • [trace] Provider request ID: req_abc123xyz789
  • [trace] Generated request hash: 29fb36cbe1742ae3
...
Not detected:
  • Provider request ID (not found in response headers)
...
What I found: [trace] Generated request hash: 29fb36cbe1742ae3
...
Detected:
  ...
  • [retries] Rate limiting detected (HTTP 429)
  • [cost] Model pricing: $0.5/1M input tokens, $1.5/1M output tokens
  • [trace] Provider request ID not found in response headers
  • [trace] Generated request hash: 29fb36cbe1742ae3
...
✓ Detected: https://api.anthropic.com
✓ Provider: anthropic
...
✓ Detected: https://generativelanguage.googleapis.com
✓ Provider: gemini
...
- **0** - SUCCESS only (all checks passed, no warnings or errors)
- **1** - WARNING or ERROR (any issues detected, including non-critical)
- **2** - Cannot run (configuration error, invalid arguments, missing TTY)

---

## Notes

1. **"Generated by AI Patch — re-run: npx ai-patch"** appears at the start and end of each run for easy re-execution.
