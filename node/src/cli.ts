...
function printDiagnosis(reportData: any): void {
  ...
  const allFindings: Array<[number, string, string, string]> = [];
  for (const checkName in checks) {
    ...
    for (const finding of findings) {
      ...
      allFindings.push([severityRank, severity, checkName, message]);
    }
  }

  // Sort by severity and take top 5
  allFindings.sort((a, b) => a[0] - b[0]);
  const topFindings = allFindings.slice(0, 5);

  if (topFindings.length > 0) {
    console.log('\nTop findings:');
    for (const [, severity, checkName, message] of topFindings) {
